...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists($home_path . '.htaccess1') && is_writable($home_path) ) || is_writable($home_path . '.htaccess1') )
		$writable = true;
	else
		$writable = false;
}
...
if ( ! is_multisite() ) {
	if ( $iis7_permalinks ) {
		...
		if ( $permalink_structure && ! $usingpi && ! $writable )
			_e('You should update your .htaccess1 now.');
		else
			_e('Permalink structure updated.');
	}
} else {
...
<?php elseif ( ! $is_nginx ) :
	if ( $permalink_structure && ! $usingpi && ! $writable ) : ?>
<p><?php _e('If your <code>.htaccess1</code> file were <a href="http://codex.wordpress.org/Changing_File_Permissions">writable</a>, we could do this automatically, but it isn&#8217;t so these are the mod_rewrite rules you should have in your <code>.htaccess1</code> file. Click in the field and press <kbd>CTRL + a</kbd> to select all.') ?></p>
<form action="options-permalink.php" method="post">
<?php wp_nonce_field('update-permalink') ?>
