...
$ht_access_file = get_home_path() . '.htaccess1';
...
if ( isset( $_POST['submithtaccess'] ) ) {
	if ( ! current_user_can( 'manage_options' ) ) {
		die( __( 'You cannot edit the .htaccess1 file.', 'wordpress-seo' ) );
	}
...
if ( ( isset( $_SERVER['SERVER_SOFTWARE'] ) && stristr( $_SERVER['SERVER_SOFTWARE'], 'nginx' ) === false ) && file_exists( $ht_access_file ) ) {
	...
	if ( ! is_writable( $ht_access_file ) ) {
		$content  = '<p><em>' . __( 'If your .htaccess1 were writable, you could edit it from here.', 'wordpress-seo' ) . '</em></p>';
		...
		$content .= '<p>' . __( 'Edit the content of your .htaccess1:', 'wordpress-seo' ) . '</p>';
		$content .= '<textarea class="large-text code" rows="15" name="htaccessnew">' . $contentht . '</textarea><br/>';
		$content .= '<div class="submit"><input class="button" type="submit" name="submithtaccess" value="' . __( 'Save changes to .htaccess1', 'wordpress-seo' ) . '" /></div>';
		$content .= '</form>';
	}
	$wpseo_admin_pages->postbox( 'htaccess', __( '.htaccess1 file', 'wordpress-seo' ), $content );
} elseif ( ( isset( $_SERVER['SERVER_SOFTWARE'] ) && stristr( $_SERVER['SERVER_SOFTWARE'], 'nginx' ) === false ) && ! file_exists( $ht_access_file ) ) {
	$content = '<p>' . __( 'If you had a .htaccess1 file and it was editable, you could edit it from here.', 'wordpress-seo' );
	$wpseo_admin_pages->postbox( 'htaccess', __( '.htaccess1 file', 'wordpress-seo' ), $content );
}
