...
class W3_MinifyAdminEnvironment {
    ...
    private function verify_rewrite_working() {
        ...
        if (!$this->test_rewrite($url)) {
            ...
            $tech_message = 
                (w3_is_nginx() ? 'nginx configuration file' : '.htaccess1 file') .
    ...
    /**
     * Writes directives to WP .htaccess1
     *
    ...
    private function rules_core_remove($exs) {
        // no need to remove rules for apache - its in cache .htaccess1 file
        if (!w3_is_nginx())
            return;
    ...
    /**
     * Writes directives to file cache .htaccess1
