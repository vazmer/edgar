...
function wpcf7_checkbox_shortcode_handler( $tag ) {
	...
	foreach ( (array) $tag->values as $key => $value ) {
		...
		if ( $label_first ) { // put label first, input last
			$item = sprintf(
				'<div class="wpcf7-list-item-label">%1$s</div>&nbsp;<input %2$s />',
				esc_html( $label ), $item_atts );
		} else {
			$item = sprintf(
				'<input %2$s />&nbsp;<div class="wpcf7-list-item-label">%1$s</div>',
				esc_html( $label ), $item_atts );
		}
	...
	$html = sprintf(
		'<div class="wpcf7-form-control-wrap %1$s"><div %2$s>%3$s</div>%4$s</div>',
...
<div style="font-size: smaller"><?php echo esc_html( __( "* One choice per line.", 'contact-form-7' ) ); ?></div>
</td>
